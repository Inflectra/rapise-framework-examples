	...
	function IsDomCustomGrid(evt, element, eventOpts, objName, description, flavor, items)
	{
		...
		function _getCell(el)
		{
			...
			if (cellEl && rowEl)
			{
				...
				// Get column name
				var column = __doDOMQueryXPath(root, "./thead/tr/th[" + (colNum + 1) + "]/div", false);
				...
				// If no text calculate row index
				if (!cell.row)
				{
					var rowNum = __doDOMQueryXPath(rowEl, "count(./preceding-sibling::*)", true);
		 ...
		 *
		 */
		if(root=__hasParentWithAttr(element, 'role', "grid"))
		{
			// TODO
