...
{
	var text = "";
	var rows = this._DoDOMQueryXPath('row path>');
	for(var rowIndex = 0; rowIndex < rows.length; rowIndex++)
	{
		var row = rows[rowIndex];
		var cells = row._DoDOMQueryXPath('<cell path>');
...
{
	var res = this.doQuery(/**xpath*/ "count(<header path>)", /**timeout*/ 0, /**return numeric result*/ true);
...
function DomCustomGridBehavior_GetColumnName(/**number*/columnIndex)  /**string*/ 
{
	...
	var res = this._DoDOMQueryXPath(/**xpath*/ "<path to the column>");
	if (res && res.length)
	{
...
function DomCustomGridBehavior_GetColumnIndex(/**string*/columnName)  /**number*/ 
{
	...
	var columns = this._DoDOMQueryXPath("<path to the column>");
...
{
	var res = this.doQuery(/**xpath*/ "count(<row path>)", /**timeout*/ 0, /**return numeric result*/ true);
...
{
	var res = this.doQuery("count(<path to selected row>/preceding-sibling::tr)", 0, true);
	if(res && res.length)
	{
		return res[0];
	}
...
{
	var res = this.doQuery("count(<path to selected row>)", 0, true);
	if(res && res.length)
	{
		return res[0];
	}
...
function DomCustomGridBehavior_GetSelectedRows(/**boolean*/asArray, /**string*/separator)  /**array|string*/ 
{
	...
	{
		if (rows[i]._DoDOMGetAttribute("<attributeName>").indexOf("<attributeValue>") != -1)
		{
			selectedRows.push(i);
		}
	}	
...
function DomCustomGrid_FindCell(obj, row, col)
{
	...
	{
		cells = obj._DoDOMQueryXPath("<row path>[" + (row + 1) + "]/<cell path>[" + (col + 1) + "]");
	}
	else if (typeof(row) == "string")
	{
		cells = obj._DoDOMQueryXPath("<cell path>[" + (col + 1) + "][normalize-space(.//text())='" + row + "']");
	}
